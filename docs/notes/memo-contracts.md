...
- Con: memos get large and noisy. Nodes may depend on fields they shouldn't. Tight coupling.
...
**Likely answer:** Transformation as the default. Accumulation fields (e.g., original outline, cycle count) are explicitly declared as pass-through in the schema rather than implicitly carried.

### Memo Format

What format are memos in? Options:
- **Structured (JSON/YAML)** — machine-parseable, schema-validatable.
- **Markdown with frontmatter** — human-readable, structured metadata in YAML, prose content in the body.
- **Free text** — maximum flexibility for LLM-to-LLM communication, minimum structure for mechanical nodes.

Mechanical nodes (mechanism, router, fan-out, fan-in) need structured fields. Agent and judgment nodes produce natural language. The format may need to accommodate both — structured envelope with a prose body.
